...
import createVehiclePhotoService from './createVehiclePhoto.service';
...
const createVehicleService = async (data: IVehicle, user: IAuthReq) => {
  ...
  await vehicleRepository.save(vehicle).catch((err) => {
    if (err) {
      throw new AppError(
        'Requisição incorreta. Confira os campos obrigatórios'
      );
    }
  });

  await createVehiclePhotoService(images, vehicle.id);
